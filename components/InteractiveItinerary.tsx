    ...
    <button
      ...
      className={
        selected
          ...
          : "group rounded-[1rem] border border-slate-200 bg-white p-3.5 text-left hover:bg-slate-50"
      }
    >
      ...
      {previewImageUrl ? (
        <div className="max-h-0 overflow-hidden opacity-0 transition-all duration-200 ease-out group-hover:mt-3 group-hover:max-h-40 group-hover:opacity-100 group-focus-visible:mt-3 group-focus-visible:max-h-40 group-focus-visible:opacity-100">
          <div className="overflow-hidden rounded-[0.9rem] border border-slate-200 bg-slate-100">
            <img
              src={previewImageUrl}
              alt={option.name}
    ...
    <section className="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-5 space-y-4">
        {days.map((day, dayIndex) => (
          <section
            key={`day-${dayIndex}`}
            className="rounded-[1.25rem] border border-slate-200 bg-slate-50/90 p-4"
          >
            <div className="mb-3">
              ...
              {(day.stops ?? []).map((stop, stopIndex) => {
                ...
                if (stop.kind === "stay") {
                  ...
                  return (
                    <div
                      key={key}
                      className="rounded-[1.1rem] border border-slate-200 bg-white p-3.5"
                    >
